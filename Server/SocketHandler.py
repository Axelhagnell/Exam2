...
#Function
    def startToAcceptConnection(self,port):
        ...
        self.list_of_know_clientUsernames = []
...
#starta trådet  som recievar
    ...
    def startReceiving(self,clientSocket, clientAddr):
        ...
        if resultOfLogin !=False:
            ...
            self.list_of_know_clientUsernames.append(username)
    ...
    def listenToknownClinet(self,clientSocket, clientAddr,username):#recieve och send funktion till kända klienter
        while True:
            ...
            #om någon disconnectar skrivs det i chattrutan
                ...
                self.list_of_know_clientUsernames.remove(username)
                self.sendAndShowMsg(username+" disconnected")
                self.users.inactiveUser(username)#inactiverar användaren
                return

    def kickKnownClient(self,username):
        for i in range (len(self.list_of_know_clientUsernames)):
            if self.list_of_know_clientUsernames[i] == username:
                try:
                    self.list_of_known_clientSockets[i].close()
#                    self.list_of_known_clientSockets.pop(i)
#                    self.list_of_known_clientAddr.pop(i)
#                    self.list_of_know_clientUsernames.pop(i)
                    break
                except:
                    print("funka inte")
                    break
